...
void main() {
  ...
  var timeChart = SwiftTimeChart(container: document.getElementById('timechart') as HTMLDivElement, series: [
    LineSeries(data: dataPoints1, smoothing: 1.0),
    LineSeries(data: dataPoints2, smoothing: 1.0),
    LineSeries(data: dataPoints3, smoothing: 1.0),
  ]);
  document.getElementById('timechartReload')!.onClick.listen((e) {
    for (var series in timeChart.series) {
  ...
  document.getElementById('barchartReload')!.onClick.listen((e) {
    for (var series in barChart.series) {
      series.data = generateRandomDataPoints(8);
    }
  });

  List<SwiftPieChart> pieCharts = [
    SwiftPieChart(container: document.getElementById('piechart1') as HTMLDivElement, legend: true, data: [
    ...
    SwiftPieChart(container: document.getElementById('piechart3') as HTMLDivElement, legend: true, data: [
      PieChartItem('foo', 20),
      PieChartItem('bar', 30),
      PieChartItem('faz', 15),
      PieChartItem('baz', 15),
  ...
  document.getElementById('piechartsReload')!.onClick.listen((e) {
    for (var pieChart in pieCharts) {
      for (var data in pieChart.data) {
  ...
  SwiftTimeChart(
      container: document.getElementById('tzchart3') as HTMLDivElement,
      series: [LineSeries(data: smallSet)],
